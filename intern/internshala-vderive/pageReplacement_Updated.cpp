...
int segmentLength=0;			//length of segment(The segment length is considered as 1 if not given	
		...
		for (size_t i = 0; i < totalProcesses; i++)
		{
			...
			if (instruction.find(processIDtoString) != string::npos)
			{
				...
				if (PageFault)
				{
					...
					{
						int r=rand()%3;
						pFrame = &framesInMainMemory[LRU_Array[r]->frameID];
					...
					framesInMainMemory[pPageAccessing->frameID].segmentID == segmentID))
{
				...
				if (PageFault)
				{
					...
					pPageAccessing->isAllocated = true;
					processes[i].addressSpace.LastPagesUsed.push_back(*pPageAccessing);i=n;
				}
				else
				{
					hit++;
...
int main()
{
	...
	cout << "6.Random_PageReplacement"<<endl;
	cout << "7.LRUk_PageReplacement"<<endl;
	...
	{
	cout << "\n-------------------------	Random Replacement	--------------------------\n"<<endl;
	Random_PageReplacement();
	}
	else if(z==7)
	{
	cout << "\n-------------------------	LRUk Replacement	--------------------------\n"<<endl;
	LRUk_PageReplacement();
	}
	else
